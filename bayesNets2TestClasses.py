    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if extraEdges:
            ...
            for edge in sorted(extraEdges):
                self.addMessage('    ' + str(edge))
            self.addMessage('\n')
        student_domain = studentNet.variableDomainsDict()
        gold_domain = goldNet.variableDomainsDict()
        if sorted(student_domain.items())!=sorted(gold_domain.items()):
            self.addMessage('Student solution has different domain, make sure you appropriately populate variableDomainsDict:')
            for k,v in student_domain.items():
                if k not in gold_domain:
                    self.addMessage(f"\tvariable {k} in student's variableDomainsDict but not in the solution's")
                elif gold_domain[k]!=v:
                    self.addMessage(f"\tvariable {k} has value {v} in student's variableDomainsDict but {gold_domain[k]} in the solution's")
            
            for k,v in gold_domain.items():
                if k not in student_domain:
                    self.addMessage(f"\tvariable {k} is in the solution's variableDomainsDict but not in the student's")
...
class VPITest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if isinstance(studentAnswer,numbers.Number) :#== float:
